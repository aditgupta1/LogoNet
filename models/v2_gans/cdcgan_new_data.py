...
loss_dir = os.path.join(data_path, "losses")
...
train_loader, shape = data.make_32x32_dataset("logo", batch_size, pin_memory=use_gpu)
...
for epoch_idx in range(epochs):
    ...
    for batch_idx, (train_x, train_y) in enumerate(train_loader):
        ...
        if batch_idx % print_every == 0:
            print(
                "\t{} ({} / {}) mean D(fake) = {:.4f}, mean D(real) = {:.4f}".format(
            ...
            g_out = (
                ...
                .data.view(SAMPLE_SIZE, nc, 32, 32)
                .cpu()
            )
            save_image(g_out, "{}/{}_{}.png".format(samples_dir, epoch_idx, batch_idx))

    print(
        "Epoch {} - D loss = {:.4f}, G loss = {:.4f}".format(epoch_idx, d_loss, g_loss)
    )
    if epoch_idx % save_every == 0:
        torch.save(
            {"state_dict": model_d.state_dict()},
            "{}/model_d_epoch_{}.pth".format(save_dir, epoch_idx),
        )
        torch.save(
            {"state_dict": model_g.state_dict()},
            "{}/model_g_epoch_{}.pth".format(save_dir, epoch_idx),
        )
        losses.append([d_loss, g_loss])
        np_loss = np.asarray(losses)
        np.savetxt(
            "{}/losses_{}.csv".format(loss_dir, epoch_idx), np_loss, delimiter=","
        )
